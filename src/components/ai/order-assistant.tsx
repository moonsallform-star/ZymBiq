...
function ContactForm({ onSubmit, isPending }: ContactFormProps) {
  // Read WhatsApp config from siteConfig
  const { data: siteConfig } = useSiteConfig();
  const whatsappNumber = siteConfig.communication.whatsappNumber ?? '';
  const whatsappEnabled = siteConfig.communication.whatsappEnabled;
    ...
    <div className="border-t border-border pt-6 mt-2">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        ...
        <div className="flex gap-2">
          {/* WhatsApp button — only shown when enabled and number is configured */}
          {whatsappEnabled && (
            <a
              href={whatsappHref}
              target="_blank"
